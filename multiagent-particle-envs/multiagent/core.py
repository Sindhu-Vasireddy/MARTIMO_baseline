...
class World(object):
    ...
    def step(self):
        ...
        for landmark in self.landmarks:
            landmark.gtst= GroundTruthPath([GroundTruthState(landmark.model.function(landmark.gtst, noise=False, time_interval=timedelta(seconds=1)),
                             timestamp=self.start_time + timedelta(seconds=self.dt))],
                            id=f"id{j}")
            landmark.gtpt.append(GroundTruthState(landmark.model.function(landmark.gtst, noise=False, time_interval=timedelta(seconds=1)),
                             timestamp=self.start_time + timedelta(seconds=self.dt)))
            j+=1
